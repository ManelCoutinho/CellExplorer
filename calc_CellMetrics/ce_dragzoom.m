...
%==========================================================================
    function Setup()
        ...
        mDragShiftStep = 0;         % step dragging on keys
        ...
        mZoomMaxPow = 5;            % max zoom percent 10 ^ mZoomMaxPow
...
%==========================================================================
    function WindowScrollWheelFcn2D(src, evnt)        %#ok
        ...
        um_axes = get(hAx,'CurrentPoint');
        b = get(hAx,'Xlim');
        c = get(hAx,'Ylim');
        cursorPosition = [um_axes(1,1);um_axes(1,2)];
        
        % if fIsEnableControl
        if cursorPosition(1) > b(1) && cursorPosition(1) < b(2) && cursorPosition(2) > c(1) && cursorPosition(2) < c(2)
            ZoomMouse(direction);
            PointerCrossUpdate();
            % end
...
%==========================================================================
    function RubberBandBegin()
        ...
        if (~fIsRubberBandOn && ~fIsMagnifierOn)
            ...
            mRubberBand = struct(...
                'obj',	[patch('Parent', hAx), patch('Parent', hAx), text('Parent', hAx), text('Parent', hAx)], ...
                'x1',  	acx, ...
                'y1',  	acy, ...
                'x2',  	acx, ...
                'y2',  	acy);
...
%==========================================================================
    function RubberBandSetPos()
        ...
        try
            set(mRubberBand.obj(1:2), ...
                'XData', [x1 x2 x2 x1], ...
                'YData', [y1 y1 y2 y2]);
            if isfield(hFig.UserData,'scalingFactor')
                set(mRubberBand.obj(3), 'String', [num2str([x2-x1]*hFig.UserData.scalingTemporalFactor,3),' sec'], 'Position', [x2 y1 0]);
                set(mRubberBand.obj(4), 'String', [num2str([y2-y1]/(hFig.UserData.scalingFactor)*1000,3),' mV'], 'Position', [x1 y2 0]);
            end
        end
    end
%--------------------------------------------------------------------------
