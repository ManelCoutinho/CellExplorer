function CheckEegStates(FileBase, varargin)
    ...
    % Channels contains channel numbers starting from 1 !!
    % if no Channels are specified, and you didn't create FileBase.eegseg.par
    % file with channels (NB: starting from 0, sorry for confusion) that you want to use, it will start
    ...
    %
    %Last modification:
    %Evgeny Resnik 03.2016: HmmStateSegment, lines 37-45, DownThrd is set smaller than UpThrd in SchmittTrigger
    %
    ...
    %NOTE: A toolbox chronux_2_12 shadows some unknown yet function used in CheckEegtates, so that it stops to work.
    % rmpath(genpath('/storage/share/matlab/Third-Party_Toolboxes/chronux_2_12'));
    ...
    if ~FileExists([FileBase '.eegseg.mat']) | Compute == 1
        ...
        if FileExists([FileBase '.lfp'])
            try
                Eeg = LoadBinary_old([FileBase '.lfp'], Channels, Par.nChannels, 4)';
            catch
                Eeg = LoadBinary_old([FileBase '.lfp'], Channels, Par.nChannels, 2)';
            end
        elseif FileExists([FileBase '.eeg'])
            try
                Eeg = LoadBinary_old([FileBase '.eeg'], Channels, Par.nChannels, 4)';
            catch
                Eeg = LoadBinary_old([FileBase '.eeg'], Channels, Par.nChannels, 2)';
            end
